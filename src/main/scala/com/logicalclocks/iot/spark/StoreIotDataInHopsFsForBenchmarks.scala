...
object StoreIotDataInHopsFsForBenchmarks extends App {

  override def main(args: Array[String]): Unit = {
    val log = LogManager.getLogger(getClass)
    log.setLevel(Level.INFO)
    log.info("Starting StoreIotDataInHopsFsForBenchmarks")
    ...
    measurementsDFs foreach { case (t, df) => df.writeStream.format("parquet").
      option("path", "/Projects/" + Hops.getProjectName + "/Resources/iot-benchmarks/data/" + t).
      option("checkpointLocation", "/Projects/" + Hops.getProjectName + "/Resources/iot-benchmarks/checkpoint/" + t).
      partitionBy("endpointClientName").
      start()
    }

    spark.streams.awaitAnyTermination()
    log.info("Shutting down StoreIotDataInHopsFsForBenchmarks job")
    spark.close()
